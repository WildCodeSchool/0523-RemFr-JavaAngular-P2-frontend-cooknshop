...
export class AjoutCategorieRecetteComponent {
  ...
  initCategories() {
    this.ApiCallService.GetResponse('categories').subscribe((categorie) => {
    ...
    if (this.formulaire && this.formulaire.valid) {
      ...
      if (selectedRecipe) {
        ...
        if (selectedCategory) {
          ...
          this.http
            .post(
              environment.apiUrl + `/recipes/${selectedRecipeId}/categories/${selectedCategoryId}/recipecategories`,
              formData
            )
            .subscribe((response) => {
